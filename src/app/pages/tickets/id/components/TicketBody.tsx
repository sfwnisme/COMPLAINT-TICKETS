...
function TheTopicTab() {
  console.log('rendered ||||| topic tab')
  return (
...
function AttachmentsTab() {
  return (
    <div className={S['ticket-page__attachments-tab']}>
      <img src="https://www.officewarehouse.com.ph/__resources/_web_data_/products/products/images/4640.jpg" alt="" />
      <img src="https://s7d9.scene7.com/is/image/OCProduction/ta50ac_g?$large$" alt="" />
      <img src="https://cdn.images.fecom-media.com/FE00008922/images/HE1501861_1501861-P.jpg?width=578&height=578&scale=UpscaleCanvas&anchor=MiddleCenter" alt="" />
    </div>
  )
}
...
export default function TicketBody() {
  ...
  const renderTabs = tabsNames?.map((tabName) => (
    <span
      ...
      id="the_topic"
      onClick={() => handleTabsChanging(tabName)} key={tabName} onKeyDown={() => handleTabsChanging(tabName)}>
      {tabName.replace('_', ' ')}
    </span>
  ))
  ...
  return (
    <article className={S['ticket-page__body']}>
      ...
      <div className={S['ticket-page__tab-container']}>
        <Shift fallback={<TheTopicTab />}>
          <Case when={tab === 'the_topic'}>
            <TheTopicTab />
          </Case>
          <Case when={tab === 'conversation'}>
            <ConversationTab />
          </Case>
          <Case when={tab === 'attachments'}>
            <AttachmentsTab />
          </Case>
          <Case when={tab === 'history'}>
            <HistoryTab />
          </Case>
        </Shift>
      </div>
